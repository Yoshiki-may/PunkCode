...
- **B案**: サーバー計算（専用エンドポイント）
  - メリット: 計算ロジックが集中、正確性保証
  - デメリット: サーバー負荷増

**推奨**: B案（サーバー計算）で専用エンドポイント追加
...
**質問**: アラートはフロント計算 or サーバー計算？

**選択肢**:
- **A案**: フロント計算
  - タスク/承認/コメント/契約を取得してフロントで集計
- **B案**: サーバー計算（専用エンドポイント）
  - `/alerts?client_id=client_a`
...
**選択肢**:
- **A案**: 設ける（例: 1ユーザーあたり1000 req/hour）
- **B案**: 設けない（初期リリース）

**推奨**: Phase 11でA案導入。Phase 10では非採用。
...
- [ ] **認証**: Supabase Auth統合、JWT Custom Claims設定
- [ ] **RLS**: 全6テーブルのRLSポリシー実装
- [ ] **エンドポイント**: 全リソースのCRUD実装
  - [ ] GET /clients
  - [ ] POST /clients
  - [ ] PATCH /clients/{clientId}
  - [ ] GET /tasks
  ...
  - [ ] POST /notifications/mark-all-read
  - [ ] DELETE /notifications/clear-all
- [ ] **Incremental Pull**: `since`パラメータ対応（全リソース）
- [ ] **ページング**: Cursor方式実装
...
- [ ] **autoPull**: Incremental Pull統合
- [ ] **Outbox**: 失敗時のリトライ
- [ ] **エラーハンドリング**: 401/403/404/500対応
...
**End of API Contract**  
**Next Action**: バックエンド実装開始 → OpenAPI仕様生成 → フロントエンド統合
